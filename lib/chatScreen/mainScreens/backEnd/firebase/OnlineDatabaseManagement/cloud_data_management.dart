...
import 'package:medskin/chatScreen/mainScreens/backEnd/localDatabase_Manage.dart';
...
class CloudStoreDataManagement {
  ...
  Future<bool> checkThisUserAlreadyPresentOrNot({ String userName}) async {
    try {
  ...
  Future<bool> registerNewUser(
      {String userName, String userAbout, String userEmail}) async {
    try {
  ...
  Future<bool> userRecordPresentOrNot({String email}) async {
    try {
  ...
  Future<Map<String, dynamic>> getTokenFromCloudStore({String userMail}) async {
    try {
      final DocumentSnapshot<Map<String, dynamic>> documentSnapshot =
      await FirebaseFirestore.instance
            ...
            if (currentUserEmail != queryDocumentSnapshot.id)
              _usersDataCollection.add({
                ...
                '${queryDocumentSnapshot.get(
                    "user_name")}[user-name-about-divider]${queryDocumentSnapshot
                    .get("about")}',
              });
          });
  ...
  Future<List<dynamic>> currentUserConnectionRequestList({String email}) async {
    try {
  ...
  Future<void> changeConnectionStatus({
    String oppositeUserMail,
    String currentUserMail,
    String connectionUpdatedStatus,
    List<dynamic> currentUserUpdatedConnectionRequest,
  ...
  fetchRealTimeMessages() async {
    try {
      return FirebaseFirestore.instance
          .doc(
          '${this._collectionName}/${FirebaseAuth.instance.currentUser.email
              .toString()}')
          .snapshots();
    } catch (e) {
      print('Error in Fetch Real Time Data : ${e.toString()}');
  ...
  // Future<void> sendMessageToConnection({String connectionUserName, Map<String,
  //     Map<String,
  //         String>> sendMessageData, ChatMessageTypes chatMessageTypes}) async {
  //   try {
  //     final localDatabase _localDatabase = localDatabase();
  //
  //     final String currentUserEmail = FirebaseAuth.instance.currentUser!
  //         .email;
  //
  ...
  //
  //     print(
  //         "Data checking: ${connectedUserData[FirestoreFieldConstants()
  //             .connections]}");
  //
  //     await FirebaseFirestore.instance
  //         .doc("${this._collectionName}/$_getConnectedUserEmail")
...
//     Future<void> removeOldMessages({ String connectionEmail}) async {
//       try {
...
//
//         print(
//             "After Remove: ${connectedUserData[FirestoreFieldConstants()
//                 .connections]}");
//
//         await FirebaseFirestore.instance
//             .doc("${this._collectionName}/$currentUserEmail")
...
//     }
//   }
  Future<List<Map<String, dynamic>>> getAllUserList(
      {String currentUserEmail}) async {
    try {
      final QuerySnapshot<
          Map<String, dynamic>> querySnapshot = await FirebaseFirestore.instance
          .collection(this._collectionName).get();
      List<Map<String, dynamic>> _userDataCollection = [];
      querySnapshot.docs.forEach((
          QueryDocumentSnapshot<Map<String, dynamic>> queryDocumentSnapshot) {
        if (currentUserEmail != queryDocumentSnapshot.id)
          _userDataCollection.add({
            queryDocumentSnapshot.id: '${queryDocumentSnapshot.get(
                "user_name")}[user-name-about-divider]${queryDocumentSnapshot
                .get("creation_date")}',
          });
      });
      print(_userDataCollection);
  ...
  Future<Map<String, dynamic>> _getCurrentAccountAllData({String email}) async {
    try {
      final DocumentSnapshot<
          Map<String, dynamic>> documentSnapshot = await FirebaseFirestore
          .instance.doc('${this._collectionName}/$email').get();
  ...
  Future<List<dynamic>> _currentUserConnectionList({String email}) async{
    try{
      ...
      print("Error in currentUserCollectionLIst: ${e.toString()}");
      return[];
    }
  }
}